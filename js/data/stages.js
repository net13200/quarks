    ...
    },
    {
        ...
        levels: [
            ...
            },
            {
                ...
                quizDesc: "q0 = |+⟩, q1 = |+⟩. Implement a CP(π/2) phase gate.",
        ...
        levels: [
            ...
            },
            {
                ...
                variational: {
                    ...
                    showOptimizer: true,
                    costLabel: 'Fidelity',
                    task: "The phase separator CX·RZ(γ)·CX encodes the Max-Cut cost into the phases of the quantum state — cut states |01⟩ and |10⟩ gain a phase e^{iγ} relative to non-cut states. Tune γ to π/2 ≈ 1.57 to reach the optimal phase-encoded state, primed for the mixer in the next level.",
                },
                lesson: "**Why U_C(γ) = e^{iγ(I − Z₁Z₂)/2}?**\n\nThe Pauli Z operator has eigenvalue +1 on |0⟩ and −1 on |1⟩. So the product Z₁Z₂ measures whether the two qubits agree:\n\n<table style='border-collapse:collapse;width:100%;margin:10px 0;font-size:0.85rem;'><thead><tr style='color:#94a3b8;border-bottom:1px solid #334155;'><th style='padding:5px 10px;text-align:left;'>state</th><th style='padding:5px 10px;text-align:center;'>Z₁Z₂</th><th style='padding:5px 10px;text-align:center;'>I − Z₁Z₂</th><th style='padding:5px 10px;text-align:center;'>cut?</th></tr></thead><tbody><tr style='border-bottom:1px solid #1e293b;'><td style='padding:5px 10px;font-family:monospace;color:#e2e8f0;'>|00⟩</td><td style='padding:5px 10px;text-align:center;'>+1</td><td style='padding:5px 10px;text-align:center;'>0</td><td style='padding:5px 10px;text-align:center;color:#64748b;'>no</td></tr><tr style='border-bottom:1px solid #1e293b;background:rgba(45,212,191,0.05);'><td style='padding:5px 10px;font-family:monospace;color:#e2e8f0;'>|01⟩</td><td style='padding:5px 10px;text-align:center;'>−1</td><td style='padding:5px 10px;text-align:center;'>2</td><td style='padding:5px 10px;text-align:center;color:#2dd4bf;'>yes</td></tr><tr style='border-bottom:1px solid #1e293b;background:rgba(45,212,191,0.05);'><td style='padding:5px 10px;font-family:monospace;color:#e2e8f0;'>|10⟩</td><td style='padding:5px 10px;text-align:center;'>−1</td><td style='padding:5px 10px;text-align:center;'>2</td><td style='padding:5px 10px;text-align:center;color:#2dd4bf;'>yes</td></tr><tr><td style='padding:5px 10px;font-family:monospace;color:#e2e8f0;'>|11⟩</td><td style='padding:5px 10px;text-align:center;'>+1</td><td style='padding:5px 10px;text-align:center;'>0</td><td style='padding:5px 10px;text-align:center;color:#64748b;'>no</td></tr></tbody></table>\n\n**(I − Z₁Z₂)/2** is the edge-cut indicator: 0 when not cut, 1 when cut. The exponential e^{iγ(I−Z₁Z₂)/2} then applies:\n* **Not cut** → e^{i·γ·0} = 1 (no phase change)\n* **Cut** → e^{i·γ·1} = e^{iγ} (relative phase boost)\n\nThis is the cost oracle: it tags cut states with a phase without touching any amplitudes. That's why the measurement probabilities stay flat at 25% each — phases are invisible until the mixer converts them into amplitude differences.\n\n**Why does CX · RZ(γ) · CX implement this?**\n\nThe CX gate has a key property: it conjugates Z on the target qubit into Z⊗Z:\n\n**CX · Z₂ · CX = Z₁Z₂**\n\nTo see why: CX maps |x,y⟩ → |x, x⊕y⟩. Acting with Z₂ in the middle gives phase (−1)^{x⊕y} = (−1)^{x+y}, which is exactly Z₁Z₂.\n\nSince CX conjugates Z₂ into Z₁Z₂, it does the same for any function of Z₂. In particular, RZ(γ) = e^{−iγZ/2}, so:\n\n**CX · RZ(γ)₂ · CX = e^{−iγ Z₁Z₂/2}**\n\nAnd e^{−iγZ₁Z₂/2} equals e^{iγ(I−Z₁Z₂)/2} up to an unobservable global phase.\n\n**The intuition:** the first CX tells qubit 2 about qubit 1's state before the rotation. The RZ now effectively sees the joint parity of both qubits. The second CX undoes the entanglement, leaving only the phase. The CNOT sandwich is the universal trick for turning a single-qubit Z rotation into a two-qubit ZZ interaction.",
                hint: "The optimal γ is π/2 ≈ 1.57. At this angle the cut states |01⟩ and |10⟩ have gained phase e^{iπ/2} = i relative to non-cut states — the perfect starting point for the mixer.",
            },
            {
                name: "14.3: The Mixer",
                ...
                hint: "Try γ ≈ 1.57 (π/2) and β ≈ 2.36 (3π/4). Or click 'Run Optimizer' to let gradient ascent find the maximum.",
            },
            {
                ...
                variational: {
                    numQubits: 3,
                    params: [
                        { id: 'gamma', label: 'γ (phase)', min: 0, max: 3.14159, init: 1.32 },
                        { id: 'beta',  label: 'β (mixer)', min: 0, max: 3.14159, init: 1.32 }
                    ],
                    template: [
                        ['H0','H1','H2'],
                        ['CX01'],
